  ...
  class Scanner
    ...
    def call
      ...
      Dir.glob(search_pattern).each do |file_path|
        ...
        metadata_hash = Flatplan::Adapters::ExifToolRunner.call(file_path)
        image_key = metadata_hash[:image_key]

        db_records[image_key] = metadata_hash
      end

      # 3. Construct an isolated filename inside the system shared tmp folder
      cache_file_path = File.join(Dir.tmpdir, "flatplan_photostore_cache.json")
